...
const char *GpuRenderOgl::fragBase = R"(
    ...
    uniform sampler2D lutD0;
    uniform sampler2D lutD1;
    uniform sampler2D lutFr;
    uniform sampler2D lutRb;
    uniform sampler2D lutRg;
    uniform sampler2D lutRr;
    uniform sampler2D lutSp;
    uniform sampler2D lutDa;
    ...
    float readLut(sampler2D lut, int i) {
        float idx = (lutAbsFlags[i] != 0) ? abs(fragIdxs[lutInputs[i]]) : fragIdxs[lutInputs[i]];
        return texture(lut, vec2((idx * 127.0 + 128.0) / 255.0, 0.5)).r * lutScales[i];
    }
    ...
    void updateFrag() {
        ...
        for (int i = 0; lightMap[i] >= 0; i++) {
            ...
            vec3 halfVec = normalize((lightVec + vtxViewVec) / 2.0);
            ...
            if ((lutMask & (1 << (8 + id))) != 0) {
                ...
                sp = texture(lutSp, vec2((idx * 127.0 + 128.0) / 255.0, (float(id) + 0.5) / 8.0)).r * lutScales[2];
            }
            if ((lutMask & (1 << (16 + id))) != 0) {
                float idx = lightAtten[id][0] + gl_FragCoord.z * lightAtten[id][1];
                sp *= texture(lutDa, vec2(idx, (float(id) + 0.5) / 8.0)).r;
            }
...
GpuRenderOgl::GpuRenderOgl(Core &core): core(core) {
    ...
    // Allocate light LUTs as 2D textures (GLES has no 1D textures): the six
    // scalar LUTs are 256x1 and the two array LUTs (SP, DA) are 256x8, one row
    // per light. R16F keeps GL_LINEAR filtering core in GLES 3.0 (R32F would
    // need OES_texture_float_linear) and 256-entry light LUTs don't need fp32.
    for (int i = 0; i < 8; i++) {
        glActiveTexture(GL_TEXTURE0 + LUT_D0 + i);
        glBindTexture(GL_TEXTURE_2D, textures[LUT_D0 + i]);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R16F, 0x100, (i < 6) ? 1 : 8, 0, GL_RED, GL_FLOAT, nullptr);
    }
}
...
void GpuRenderOgl::flushBuffers(uint32_t mod) {
    ...
    // Read the whole color buffer back as RGBA8 (the one format GLES guarantees
    // for glReadPixels) and pack each pixel into the 3DS color format in memory.
    // GLES returns bytes R,G,B,A per pixel, i.e. uint32 little-endian A<<24|B<<16|G<<8|R.
    uint16_t w = bufWidth, h = bufHeight;
    uint32_t *data = new uint32_t[w * h];
    glReadPixels(0, 0, w, h, GL_RGBA, GL_UNSIGNED_BYTE, data);
    for (int y = 0; y < h; y++) {
        for (int x = 0; x < w; x++) {
            uint32_t c = data[y * w + x];
            uint8_t r = c, g = c >> 8, b = c >> 16, a = c >> 24;
            uint32_t ofs = getSwizzle(x, y, w);
            switch (colbufFmt) {
            case COL_RGBA8:
                core.memory.write<uint32_t>(ARM11, colbufAddr + ofs * 4,
                    ((uint32_t)r << 24) | ((uint32_t)g << 16) | ((uint32_t)b << 8) | a);
                break;
            case COL_RGB8:
                core.memory.write<uint8_t>(ARM11, colbufAddr + ofs * 3 + 0, b);
                core.memory.write<uint8_t>(ARM11, colbufAddr + ofs * 3 + 1, g);
                core.memory.write<uint8_t>(ARM11, colbufAddr + ofs * 3 + 2, r);
                break;
            case COL_RGB565:
                core.memory.write<uint16_t>(ARM11, colbufAddr + ofs * 2,
                    ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3));
                break;
            case COL_RGB5A1:
                core.memory.write<uint16_t>(ARM11, colbufAddr + ofs * 2,
...
void GpuRenderOgl::updateBuffers() {
    ...
    if (readDirty & BIT(0)) {
        // Decode the 3DS color buffer from memory into an RGBA8 image and upload
        // it. Keeping the GL color texture RGBA8 regardless of the 3DS format
        // means the FBO stays color-renderable and glReadPixels-able; round-trip
        // precision is preserved when flushBuffers packs it back.
        uint16_t w = bufWidth, h = bufHeight;
        uint32_t *data = new uint32_t[w * h]; // RGBA8: little-endian A<<24|B<<16|G<<8|R
        glActiveTexture(GL_TEXTURE0 + TEX_BUFFER);
        for (int y = 0; y < h; y++) {
            for (int x = 0; x < w; x++) {
                uint32_t ofs = getSwizzle(x, y, w);
                ...
                switch (colbufFmt) {
                case COL_RGBA8: {
                    uint32_t c = core.memory.read<uint32_t>(ARM11, colbufAddr + ofs * 4);
                    r = c >> 24; g = c >> 16; b = c >> 8; a = c;
                    break;
                }
                case COL_RGB8:
                    b = core.memory.read<uint8_t>(ARM11, colbufAddr + ofs * 3 + 0);
                    g = core.memory.read<uint8_t>(ARM11, colbufAddr + ofs * 3 + 1);
                    r = core.memory.read<uint8_t>(ARM11, colbufAddr + ofs * 3 + 2);
                    break;
                case COL_RGB565: {
                    uint16_t c = core.memory.read<uint16_t>(ARM11, colbufAddr + ofs * 2);
                ...
                case COL_RGB5A1: {
                    uint16_t c = core.memory.read<uint16_t>(ARM11, colbufAddr + ofs * 2);
                    r = ((c >> 11) & 0x1F) * 0xFF / 0x1F;
                    g = ((c >> 6) & 0x1F) * 0xFF / 0x1F;
                    b = ((c >> 1) & 0x1F) * 0xFF / 0x1F;
                    a = (c & 0x1) * 0xFF;
                    break;
                }
                case COL_RGBA4: {
                ...
                data[y * w + x] = ((uint32_t)a << 24) | ((uint32_t)b << 16) | ((uint32_t)g << 8) | r;
            }
        }
...
// GLES has no GL_UNSIGNED_INT_8_8_8_8. The renderer builds RGBA8 texels as
// R<<24|G<<16|B<<8|A uint32s; byteswapping each yields little-endian R,G,B,A
// bytes for a GL_RGBA8 / GL_UNSIGNED_BYTE upload.
static void texImageRgba8(GLsizei w, GLsizei h, uint32_t *data) {
...
void GpuRenderOgl::updateTextures() {
    ...
    for (int i = 0; texDirty >> i; i++) {
        ...
        glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR_EXT, texBorders[i]); // EXT_texture_border_clamp
        if (cache) continue;
        ...
        case TEX_RGB8:
            ...
            for (int y = 0; y < h; y++, y1--)
                for (int x = 0; x < w; x++)
                    dat32[y1 * w + x] = core.memory.read<uint32_t>(ARM11, texAddrs[i] + getSwizzle(x, y, w) * 3 - 1) | 0xFF;
            texImageRgba8(w, h, dat32); // RGB8 has no alpha; the | 0xFF forces it opaque
            delete[] dat32;
            continue;
        case TEX_RGB5A1:
        ...
        case TEX_LA8: case TEX_RG8:
            ...
            for (int y = 0; y < h; y++, y1--)
                for (int x = 0; x < w; x += 2)
                    dat32[(y1 * w + x) / 2] = core.memory.read<uint32_t>(ARM11, texAddrs[i] + getSwizzle(x, y, w) * 2);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RG8, w, h, 0, GL_RG, GL_UNSIGNED_BYTE, dat32);
            delete[] dat32;
            continue;
        case TEX_L8: case TEX_A8:
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, w, h, 0, GL_RED, GL_UNSIGNED_BYTE, dat16);
            delete[] dat16;
            continue;
        case TEX_LA4:
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RG8, w, h, 0, GL_RG, GL_UNSIGNED_BYTE, dat16);
            delete[] dat16;
            continue;
...
void GpuRenderOgl::updateLuts() {
    ...
    for (int i = 0; lutDirty != 0; i++) {
        ...
        case LUT_D0:
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0x100, 1, GL_RED, GL_FLOAT, lutD0);
            continue;
        case LUT_D1:
            glActiveTexture(GL_TEXTURE0 + TEX_LUTD1);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0x100, 1, GL_RED, GL_FLOAT, lutD1);
            continue;
        case LUT_FR:
            glActiveTexture(GL_TEXTURE0 + TEX_LUTFR);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0x100, 1, GL_RED, GL_FLOAT, lutFr);
            continue;
        case LUT_RB:
            glActiveTexture(GL_TEXTURE0 + TEX_LUTRB);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0x100, 1, GL_RED, GL_FLOAT, lutRb);
            continue;
        case LUT_RG:
            glActiveTexture(GL_TEXTURE0 + TEX_LUTRG);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0x100, 1, GL_RED, GL_FLOAT, lutRg);
            continue;
        case LUT_RR:
            glActiveTexture(GL_TEXTURE0 + TEX_LUTRR);
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0x100, 1, GL_RED, GL_FLOAT, lutRr);
            continue;
        ...
        case LUT_SP4: case LUT_SP5: case LUT_SP6: case LUT_SP7:
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, i - LUT_SP0, 0x100, 1, GL_RED, GL_FLOAT, lutSp[i - LUT_SP0]);
            continue;
        ...
        case LUT_DA4: case LUT_DA5: case LUT_DA6: case LUT_DA7:
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, i - LUT_DA0, 0x100, 1, GL_RED, GL_FLOAT, lutDa[i - LUT_DA0]);
            continue;
        }
    }
}
...
void GpuRenderOgl::setTexWrap(int i, TexWrap wrapS, TexWrap wrapT) {
    ...
    switch (wrapS) {
        ...
        case WRAP_BORDER: texWrapS[i] = GL_CLAMP_TO_BORDER_EXT; break;
    ...
    switch (wrapT) {
        ...
        case WRAP_BORDER: texWrapT[i] = GL_CLAMP_TO_BORDER_EXT; return;
